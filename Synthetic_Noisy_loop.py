...
# Segment setup
steps = {"A": 100, "B": 100, "C": 550, "D": 500, "E": 150, "F": 200}
segments = []
...
imu_noisy[:, 0:3] += np.random.normal(0, 0.01, imu_noisy[:, 0:3].shape)
imu_noisy[:, 3:6] += np.random.normal(0, 0.1, imu_noisy[:, 3:6].shape)
for i in range(6):
    imu_noisy[:, i] += low_freq_noise(len(imu_noisy), std=0.02 if i < 3 else 0.05)
