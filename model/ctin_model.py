...
class CTINModel(nn.Module):
    def __init__(
        ...
        output_mode="pos"  # NEW: can be "pos" or "vel"
    ):
    ...
    def forward(self, imu_tensor):
        ...
        Returns:
            prediction: [B, T, 2] – vel or pos depending on mode
            cov_pred:   [B, T, 2] – log variance
        """
        ...
        prediction, cov_pred = self.output_heads(decoded)

        return prediction, cov_pred
