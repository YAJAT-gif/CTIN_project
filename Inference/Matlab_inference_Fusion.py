...
# ==== Predict ====
with torch.no_grad():
    pred_vel, _ = model(X_tensor)  # [N, 200, 2]
    pred_vel = pred_vel * vel_scale
    pred_vel = pred_vel.numpy()

# ==== Velocity Fusion ====
fused_vel = np.zeros((T, 2))
count = np.zeros((T, 1))
...
for i in range(len(pred_vel)):
    ...
    if end <= T:
        fused_vel[start:end] += pred_vel[i]
        count[start:end] += 1

fused_vel = fused_vel / (count + 1e-6)

# Trim ends where count is zero (just in case)
valid = count[:, 0] > 0
fused_vel = fused_vel[valid]
vel_gt = vel_gt[:len(fused_vel)]

# ==== Integrate ====
pos_gt = np.cumsum(vel_gt * dt, axis=0)
pos_pred = np.cumsum(fused_vel * dt, axis=0)

# === Ensure same length for error computation ===
min_len = min(len(pos_gt), len(pos_pred))
...
plt.xlabel("X (m)")
plt.ylabel("Y (m)")
plt.title("CTIN Trajectory with Velocity Fusion")
plt.axis("equal")
plt.grid(True)
plt.legend()
plt.tight_layout()
